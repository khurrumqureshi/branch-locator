...
function getBranches(lat,long,dist,callback){
    ...
    async.forEach(data.database.branches,function(bracnh,cb){
        ...
        if(dist>=difference){
            ...
            bracnh.image = "http://maps.googleapis.com/maps/api/staticmap?center="+loc+"&zoom=17&size=300x450&maptype=roadmap&markers=color:green%7Clabel:SCB%7C"+loc+"&sensor=false&key=AIzaSyDXwKO7CDJhA7P1NDpb35vYb2_qdp9MTSs";
